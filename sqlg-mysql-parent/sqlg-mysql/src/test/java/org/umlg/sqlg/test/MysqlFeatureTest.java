...
@CucumberOptions(
        ...
        objectFactory = MysqlFeatureTest.SqlgGraphGuiceFactory.class,
        features = {"classpath:/org/apache/tinkerpop/gremlin/test/features"},
        plugin = {"progress", "junit:target/cucumber.xml"})
public class MysqlFeatureTest {
...
//            "g_V_drop"
    ...
    private final static Map<String, String> OPT_OUT = Map.ofEntries(
            ...
            Map.entry("g_V_repeatXoutX_timesX3X_count", "To slow"),
            ...
            Map.entry("g_V_hasXk_withinXcXX_valuesXkX", "strange chars not accepted by mariadb")
    );
    ...
    private static Map<String, Object> getBaseConfiguration(final GraphData graphData) {
        return new HashMap<>() {{
            put("jdbc.url", "jdbc:mysql://localhost:" + graphDataToPort(graphData) + "/?allowPublicKeyRetrieval=true&useSSL=false");
            put("jdbc.username", "root");
            put("jdbc.password", "mysql");
        }};
    }

    public static int graphDataToPort(GraphData graphData) {
        if (graphData == null) {
            return 3310;
        }
        switch (graphData) {
            ...
            default -> {
                throw new IllegalStateException("Unhandled GraphData:" + graphData.name());
            }
        }
    ...
    public static class MariadbWorld extends SqlgWorld {
        public MariadbWorld() {
    ...
    public static final class ServiceModule extends AbstractModule {
        ...
        protected void configure() {
            bind(World.class).to(MariadbWorld.class);
        }
    }
