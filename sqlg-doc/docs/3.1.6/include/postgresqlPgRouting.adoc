...
Sqlg has support for pgrouting's https://docs.pgrouting.org/latest/en/pgr_bdDijkstra.html[*pgr_bdDijkstra*]
...
=== pgConnectedComponent

Sqlg has support for pgrouting's https://docs.pgrouting.org/latest/en/pgr_connectedComponents.html[*pgr_connectedComponents*]
[source,java,options="nowrap"]
----
@Test
public void testConnectedComponent() {
    loadPGRoutingSampleData(); #<1>

    List<Vertex> components = this.sqlgGraph.traversal().E().hasLabel("edges").pgrConnectedComponent().toList(); #<2>
    ...
    String pgr_connectedComponents = """
            ...
            """;
    assertPGConnectedComponents(components, pgr_connectedComponents); #<4>
}
----

<1> Load the sample dataset from pgrouting's documentation.
<2> Call the `pgrConnectedComponent` step.
<3> Each unique component will have a special id associated to it. The special property is `SqlgPGRoutingFactory.TRAVERSAL_COMPONENT`
<4> Assert that the result is the same as getting the components using raw sql.
