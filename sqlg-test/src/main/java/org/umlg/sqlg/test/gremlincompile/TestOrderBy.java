...
//            System.out.println(vertex.<String>value("name") + " " + vertex.<String>value("surname"));
//        }
//    }
//
//    @Test
...
//                .hasLabel("Group").as("g").order().by("name", Order.decr)
//                .out("groupNetwork").as("network").order().by("name", Order.decr)
//                .out("networkNetworkSoftwareVersion").as("nsv").order().by("name", Order.decr)
//                .out("networkSoftwareVersionNetworkNodeGroup").as("nng").order().by("name", Order.incr)
//                .out("networkNodeGroupNetworkNode").as("nn").order().by("name", Order.decr)
...
//
    ...
    public void testOrderby3() {
        ...
        List<Map<String,Vertex>> result = this.sqlgGraph.traversal().V()
                .hasLabel("A").as("a").order().by("name", Order.incr)
                .out("ab").as("b").order().by("name", Order.decr)
                .<Vertex>select("a", "b")
                .toList();
        for (Map<String, Vertex> map : result) {
            System.out.println(map.get("a").<String>value("name") + " " + map.get("b").<String>value("name"));
        }
    }
