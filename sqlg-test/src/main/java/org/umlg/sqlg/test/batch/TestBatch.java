...
public class TestBatch extends BaseTest {
    ...
    public void batchUpdatePeriod() throws InterruptedException {
        ...
        List<Vertex> vertices = this.sqlgGraph.traversal().V().hasLabel("Person").toList();
        Assert.assertEquals(10, vertices.size());
        Assert.assertEquals(period, vertices.get(0).<Duration>value("period"));
        this.sqlgGraph.tx().normalBatchModeOn();
    ...
    public void testBatchUpdateArrayString() {
        ...
        Assert.assertTrue(list.contains("name1"));
        Assert.assertTrue(list.contains("name2"));
        god.property("array", new String[]{"name3", "name4"});
        this.sqlgGraph.tx().commit();
        god = this.sqlgGraph.traversal().V(god.id()).next();
        array = god.values("array");
        Assert.assertTrue(array.hasNext());
        o = array.next();
        Assert.assertTrue(o instanceof String[]);
        list = Arrays.asList((String[]) o);
        Assert.assertEquals(2, list.size());
        Assert.assertTrue(list.contains("name3"));
        Assert.assertTrue(list.contains("name4"));
    }
    ...
    public void testBatchUpdateArrayInteger() {
        ...
        Assert.assertArrayEquals(array, new Integer[]{1, 2});
    }
    ...
    public void testBatchUpdateArrayboolean() {
        ...
        Assert.assertArrayEquals(array, new boolean[]{false, true});
    }
    ...
    public void testBatchUpdateArraybyte() {
        ...
        Assert.assertArrayEquals(array, new byte[]{5, 6});
    }

    //TODO can't work out how to convert postgres byte array resultset back to Byte
    @Test
    public void testBatchUpdateArrayByte() {
        this.sqlgGraph.tx().normalBatchModeOn();
        Byte[] bytes = {1, 3};
        ...
        god = this.sqlgGraph.traversal().V(god.id()).next();
        Byte[] array = god.value("array");
        Assert.assertArrayEquals(array, new Byte[]{6, 7});
    }
