...
//    }
    ...
    @Test
    public void testTwoGraphsOpenSameTime() {
        Vertex john = this.sqlgGraph.addVertex(T.label, "Person", "name", "john");
        Vertex deer = this.sqlgGraph.addVertex(T.label, "Person", "name", "deer");
        john.addEdge("tractor", deer);
        Configuration conf = new PropertiesConfiguration();
        conf.setProperty("jdbc.url", "jdbc:postgresql://localhost:5432/standard");
        conf.setProperty("jdbc.username", "postgres");
        conf.setProperty("jdbc.password", "postgres");
        SqlgGraph sqlgGraph1 = SqlgGraph.open(conf);
        Vertex peter = sqlgGraph1.addVertex(T.label, "Person", "name", "peter");
        Vertex smith = sqlgGraph1.addVertex(T.label, "Person", "name", "smith");
        peter.addEdge("tractor", smith);
