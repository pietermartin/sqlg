...
public class TestPgHalfvec extends BaseTest {

    @Test
    public void testPgVector() {
        this.sqlgGraph.getTopology().getPublicSchema()
                .ensureVertexLabelExist("PGVector", new HashMap<>() {{
                    put("embedding", PropertyDefinition.of(PropertyType.pghalfvec(3)));
                }});
    ...
    public void testPgVectorStreamingBulkMode() {
        ...
        this.sqlgGraph.streamVertex(T.label, "PGVector", "embedding", new PGhalfvec(new float[]{1, 2, 3}));
        this.sqlgGraph.streamVertex(T.label, "PGVector", "embedding", new PGhalfvec(new float[]{1, 2, 3}));
        this.sqlgGraph.streamVertex(T.label, "PGVector", "embedding", new PGhalfvec(new float[]{1, 2, 3}));
        this.sqlgGraph.streamVertex(T.label, "PGVector", "embedding", new PGhalfvec(new float[]{1, 2, 3}));
        this.sqlgGraph.tx().commit();
