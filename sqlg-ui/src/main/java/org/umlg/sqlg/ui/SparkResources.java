...
//        app.get("/sqlg/data/v1/topologyTree", ctx -> { // will match anything starting with /path/
//            ctx.json(SchemaResource.retrieveTopologyTree(ctx));
////            ctx.result("You are here because " + ctx.path() + " matches " + ctx.matchedPath());
//        });
//        app.get("/sqlg/data/v1/topologyTree", (req, res) -> SchemaResource.retrieveTopologyTree(req));
