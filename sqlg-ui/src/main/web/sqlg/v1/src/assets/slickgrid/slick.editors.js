...
(function ($) {
  ...
  function SelectCellEditor(args) {
    ...
    this.init = function () {
      let options = _.map(args.column.options, function (o) {
        return _.extend({
          text: o.name
        }, o);
      });
      selectList = `<SELECT tabIndex='0' class='editor-selectCell'>`;
      if (args.column['cm_column'] && !args.column['cm_column'].isRequired) {
    ...
    this.serializeValue = function () {
      return ($select.val() === defaultValue) ? defaultValue : $select.val();
    };

    this.applyValue = function (item, state) {
      let option = _.find(args.column.options, o => o.id === state);
      item[args.column.field] = !!option && !!option.id ? option.id : '';
    };
